...
def assess():
    ...
    # 誤り訂正API により 定性結果を受け取る
    r = requests.post("http://llid_flask:5001/correct", json=q)
    result = r.json()['result']
    orig = r.json()['orig']
    r = r.json()

    # errant
    q = {"orig": orig, "cor": result}
    r = requests.post("http://errant_flask:5002/errant", json=q)
    errant = r.json()
    return jsonify({"result": result, "errant": errant})

# curl -X POST -H "Content-Type: application/json" -d '{"id": "510", "composition": "bard is looking apple."}' localhost:5000/assess
