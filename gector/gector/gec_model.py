...
class GecBERTModel(object):
    ...
    def postprocess_batch(self, batch, all_probabilities, all_idxs,
        ...
        for tokens, probabilities, idxs, error_prob in zip(batch,
            ...
            edits = []
            # print('一周分！！！！！！')
            # print(tokens, probabilities, idxs, error_prob)
            ...
            # 入力文に対する訂正すべき確率が低ければスルー
            if error_prob < self.min_error_probability: # デフォルトではゼロ
                all_results.append(tokens)
                continue
            ...
            for i in range(length + 1):
                ...
                action = self.get_token_action(token, i, probabilities[i],
                                               sugg_token)
                # print('アクション:', 'tokenは', token, action)
                if not action:
                    continue
    ...
    def handle_batch(self, full_batch):
        ...
        for n_iter in range(self.iterations):
            ...
            # idxsが一つでも非ゼロなら誤りあり
            # print('これがprobabilities', probabilities)
            # print('これがidxs', idxs)
            # print('これがerror_probs', idxs)

            pred_batch = self.postprocess_batch(orig_batch, probabilities,
                                                idxs, error_probs)
            if self.log:
                print(f"Iteration {n_iter + 1}. Predicted {round(100*len(pred_ids)/batch_size, 1)}% of sentences.（入力文のうち訂正箇所を含む文がいくつあるか）")
